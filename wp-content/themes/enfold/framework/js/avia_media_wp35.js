...
*************************************************************************/
	...
	$.AviaElementBehavior.fontello_insert = function(event, selection, options)
	{
		...
		{
			$('body').avia_alert({the_class:'error', text:'请上传有效的ZIP文件。<br/>你可以在Fontello.com创建文件'});
			return;
		}
		...
		$.ajax({
			...
			error: function()
			{
				$('body').avia_alert({the_class:'error', text:'无法添加字体，因为该服务器没有响应。<br/>请重新加载该页面，然后再试一次'});
			},
			success: function(response)
			{
				...
				{
					$('body').avia_alert({the_class:'error', show:6500 , text:'不能添加字体。<br/>脚本返回下列错误： '+"<br/><br/>"+response});
				}
	...
	$.AviaElementBehavior.fontello_remove = function(event)
	{
		...
		$.ajax({
			...
			error: function()
			{
				$('body').avia_alert({the_class:'error', text:'无法删除字体，因为该服务器没有响应。<br/>请重新加载该页面，然后再试一次'});
			},
			success: function(response)
			{
				...
				{
					$('body').avia_alert({the_class:'error', text:'不能删除字体。<br/>请重新加载该页面，然后再试一次'});
				}
    ...
    $.AviaElementBehavior.config_file_insert = function(event, selection, options)
    {
        ...
        {
            $('body').avia_alert({the_class:'error', text:'请上传有效的配置文件。<br/>你可以通过点击“导出主题设置”按钮创建文件'});
            return;
        }
        ...
        $.ajax({
            ...
            error: function()
            {
                $('body').avia_alert({the_class:'error', text:'无法导入配置，因为该服务器没有响应。<br/>请重新加载该页面，然后再试一次'});
            },
            success: function(response)
            {
                ...
                {
                    $('body').avia_alert({the_class:'error', show:6500 , text:'不能导入字体文件。<br/>脚本返回以下错误：'+"<br/><br/>"+response});
                }
