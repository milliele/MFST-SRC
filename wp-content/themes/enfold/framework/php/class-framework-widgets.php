...
if (!class_exists('avia_fb_likebox'))
{
	class avia_fb_likebox extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( 'avia_fb_likebox', THEMENAME.' Facebook喜欢框', $widget_ops );
		}
...
if (!class_exists('avia_tweetbox'))
{
	class avia_tweetbox extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( 'tweetbox', THEMENAME.' Twitter小工具', $widget_ops );
		}
...
if (!class_exists('avia_newsbox'))
{
	class avia_newsbox extends WP_Widget {
		...
		function __construct()
		{
			...
			parent::__construct( 'newsbox', THEMENAME.' 最新新闻', $widget_ops );
		}
...
if (!class_exists('avia_portfoliobox'))
{
	class avia_portfoliobox extends avia_newsbox
	{
		function __construct()
		{
			...
			WP_Widget::__construct( 'portfoliobox', THEMENAME.' 最新作品', $widget_ops );
		}
	}
}
...
if (!class_exists('avia_socialcount'))
{
	class avia_socialcount extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( 'avia_socialcount', THEMENAME.' RSS链接和Twitter帐户', $widget_ops );
		}
...
if (!class_exists('avia_partner_widget'))
{
	class avia_partner_widget extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( 'avia_partner_widget', THEMENAME.' 广告小工具', $widget_ops );
		}
...
if (!class_exists('avia_one_partner_widget'))
{
	...
	class avia_one_partner_widget extends avia_partner_widget
	{
		function __construct()
		{
			...
			parent::__construct( 'avia_one_partner_widget', THEMENAME.' 大广告区域', $widget_ops );
		}
	}
}
...
if (!class_exists('avia_google_maps'))
{
	class avia_google_maps extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct('avia_google_maps', THEMENAME.' 谷歌地图小工具', $widget_ops);
        ...
        {
// 四亩地            $prefix  = is_ssl() ? "https" : "http";
// 四亩地            $api_key = avia_get_option('gmap_api');
 // 四亩地           $api_url = $prefix.'://maps.google.com/maps/api/js?v=3.24';
            
 // 四亩地           if($api_key != ""){
// 四亩地	           $api_url .= "&key=" .$api_key;
// 四亩地            }
            
 // 四亩地           wp_register_script( 'avia-google-maps-api', $api_url, array('jquery'), NULL, true);
...
if(!function_exists('avia_printmap'))
{
 ...
 // 四亩地 		if(apply_filters('avia_google_maps_widget_load_api', true, $avia_config['g_maps_widget_active']))
 // 四亩地         {
 // 四亩地 	        $prefix  = is_ssl() ? "https" : "http";
  // 四亩地            $api_key = avia_get_option('gmap_api');
   // 四亩地           $api_url = $prefix.'://maps.google.com/maps/api/js?v=3.24';
  ...
  // 四亩地            wp_register_script( 'avia-google-maps-api', $api_url, array('jquery'), NULL, true);
 // 四亩地             wp_enqueue_script( 'avia-google-maps-api' );
  // 四亩地        }
