...
if( ! class_exists( 'avia_htmlhelper' ) )
{
	class avia_htmlhelper
	{
		...
		function render_single_element( $element )
		{
			...
			if( method_exists( $this, $element['type'] ) || (isset($element['use_function']) && function_exists($element['type'])))
			{
				...
				if($element['dynamic'])
				{
					...
					$output .= '		<span class="avia_clone_loading avia_removable_element_loading avia_hidden">加载中</span>';
		...
		{	
			...
			for ($i = 0; $i < $iterations; $i++)
			{
				...
				$output  .= '	<span class="avia_clone_loading avia_hidden" href="#">加载中</span>';
		...
		{	
			...
			$output .= '<div class="avia_import_wait"><strong>导入开始。</strong><br/>请等待几秒钟，再重新载入页面。导入完会尽快通知你！ :)</div>';
     ...
     */
        function parent_setting_import($element)
        {
            ...
            $output .= '<span class="avia_style_wrap"><a href="#" class="avia_button avia_import_parent_button">导入父主题设置</a></span>';
            $output .= '<span class="avia_loading avia_import_loading_parent"></span>';
            $output .= '<div class="avia_import_parent_wait"><strong>导入开始。</strong><br/>请等待几秒钟，再重新载入页面。导入完会尽快通知你！ :)</div>';
		...
		function page_header()
		{
			...
			$output .= '					<li><a class="thickbox" onclick="return false;" href="http://docs.kriesi.at/'.avia_backend_safe_string($this->avia_superobject->base_data['prefix']).'/changelog/index.php?TB_iframe=1">更新日志</a> |</li>';
			...
			$output .= '				</ul>';
			$output .= '				<a class="avia_shop_option_link" href="#">显示所有选项 [+]</a>';
		...
		{
			$output = '<span class="avia_style_wrap"><a href="#" class="avia_button avia_button_inactive avia_submit">保存所有更改</a></span>';
			return $output;
		}
		...
		function reset_button()
		{
			...
			$output = '<span class="avia_style_wrap"><a href="#" class="avia_button avia_button_grey avia_reset">重置所有选项</a></span>';
			return $output;
		}
	...
	function dynamical_add_elements($element)
	{
		...
		$output .= '<option value="">选择元素</option>  ';
		...
		$output .= '</select>';
		$output .= '<span class="avia_select_fake_val">选择元素</span></span>';
...
######################################################################
	...
	function styling_wizard_select($element)
	{
		...
		$output .= "</select><span class='avia_select_fake_val'>默认</span></span></span>";
		$output .= "<div class='subname'>".$sub_values['name']."</div>";
		return $output;
	}
