...
/* filter menu item urls */
/* 四亩地 if(!function_exists('avia_maps_key_for_plugins'))
{
...
if(!function_exists('avia_append_search_nav'))
{
	...
	{	
	    ...
	    //HLM CHANGED BEGINS
	    if ((is_object($args) && ($args->theme_location == 'avia' || $args->theme_location == 'avia4')) || (is_string($args) && $args = "fallback_menu"))
	    //HLM CHANGED ENDS
	    {
	        global $avia_config;
	        ob_start();
	        ...
	        <p class="field-avia-link-style description description-wide avia_mega_menu avia_mega_menu_d0">
				<label for="<?php echo $key; ?>">
					...
					<select id="<?php echo $name . "-". $item_id;?>" class="widefat edit-menu-item-target" name="<?php echo $name . "[". $item_id ."]";?>">
						...
						<option value="av-menu-button av-menu-button-colored" 	<?php selected( $value,  'av-menu-button av-menu-button-colored'); ?>	><?php _e('按钮风格（彩色）' ); ?>	</option>
						<option value="av-menu-button av-menu-button-bordered" 	<?php selected( $value,  'av-menu-button av-menu-button-bordered'); ?>	><?php _e('按钮风格（框式）'); ?>	</option>
					</select>
				</label>
			</p>
...
if (!class_exists('avia_mailchimp_widget'))
{
		...
		function __construct() {
			//Constructor
			$widget_ops = array('classname' => 'avia_mailchimp_widget', 'description' => '显示Mailchimp通讯注册表单的小工具' );
			parent::__construct( 'avia_mailchimp_widget', THEMENAME.' Mailchimp通讯注册', $widget_ops );
		}
