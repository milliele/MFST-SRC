...
class avia_font_manager{
	...
	static function add_font_manager($output, $element)
	{
		...
		if(!empty($font_configs))
		{
			...
			{
				$output .= "<div class='avia-available-font' data-font='".$font_name."'><span class='avia-font-name'>字体：".$font_name."</span>";
				if(!isset($font_file['full_path'])) 
				{
					$output .= "<a href='#delete-{$font_name}' data-delete='{$font_name}' class='avia-del-font'>删除</a>";
				}
				else
				{
					$output .= "<span class='avia-def-font' data-delete='{$font_name}'>(默认字体)</span>";
				}
				$output .= "</div>";
			}
		}
