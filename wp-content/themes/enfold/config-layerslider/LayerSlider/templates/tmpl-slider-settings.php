		...
		<div class="ls-settings-contents">
			...
			<table>
				...
				<tbody class="ls-settings-popup">
					<tr class="ls-premium">
						<td colspan="3">
							...
							<div id="ls-popup-notifications">
								<?php if( ! get_option('layerslider-authorized-site', true) ) : ?><!--四亩地-->
					...
					<tr>
						<td colspan="3" class="center ls-spacer-top ls-spacer-bottom"><?php echo sprintf(__('查看%s发布%s和%s手机%s部分以设置调度、目标设备等。'), '<a href="#publish">', '</a>', '<a href="#mobile">', '</a>') ?></td>
					</tr>
					<tr>
					...
					<tr class="ls-popup-include-custom-pages">
						...
						<td colspan="2"><?php lsGetInput($sDefs['popupPagesCustom'], $sProps, array( 'placeholder' => __('逗号分隔的页面ID、标题和别名列表') )); ?></td>
					</tr>
					<tr class="ls-popup-exclude-pages">
						<td><?php _e('排除页面') ?></td>
						<td colspan="2"><?php lsGetInput($sDefs['popupPagesExclude'], $sProps, array( 'placeholder' => __('逗号分隔的页面ID、标题和别名列表') )); ?></td>
					</tr>
